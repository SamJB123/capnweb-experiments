...
// Drives a server session that processes a single ["push", ImportId, expr] frame (this fork's
// protocol carries an explicit import ID; the first client push is ID 1), returning the transport
// so the caller can inspect the reaction.
function pushFrame(expr: string, limits?: Partial<RpcLimits>): ScriptedTransport {
  let transport = new ScriptedTransport([`["push",1,${expr}]`]);
