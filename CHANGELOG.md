...
- **Merged `0.12.0-hibernation-cbor.1`**: the fire-and-forget call helpers (`__experimental_streamCall`, `__experimental_releaseCall`, `__experimental_onewayCall`) with the new `["oneway", expression]` wire message, and the `CodecTransport.receive()` per-message retention fix. See that entry below for details. Peers using `__experimental_onewayCall` must both run this version or later.
