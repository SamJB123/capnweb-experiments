...
Build locally with `pnpm run build:docs` from the repo root. The fork retains its
existing static-assets Wrangler configuration; upstream's Cloudflare account and
`capnweb.com` production/preview routes are not configured here. Configure the
fork's account and hostname before using `pnpm run deploy:docs`.
